...
void Graph::InsertOperation(const string &node_name, const string &operation_type, 
    ...
    if(operation_type == "+")
    {                            
        OperationNodeTwo* tmp = new add(node_name, nodes_[input1], nodes_[input2]);
        nodes_.insert(node_name, tmp);
    }    
    else
    {
        OperationNodeTwo* tmp = new sub(node_name, nodes_[input1], nodes_[input2]);
        nodes_.insert(node_name, tmp);
    }                                       
    //  fill, 根据operation_type的名字调用相应的运算符节点构造函数
}
...
void Graph::Eval(const string &node_name, const int &argument_num, 
    ...
    {
        auto ptr = dynamic_cast<PlaceholderNode*>(nodes_[argument_name_list_[i]]);
        ptr->set_val(argument_value_list_[i]);
    }    
    // fill, 调用相应的placeholder的set_val函数
